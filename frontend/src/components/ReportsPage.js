...
const ReportsPage = ({ userRole }) => {
  ...
  }, [fetchTopSellingProducts, isAdmin, isStaff]);

  const chartData = {
    labels: topSellingProducts.map((product) => product.sku),
    datasets: [
      {
        label: "Quantity Sold",
        data: topSellingProducts.map((product) => product.total_quantity_sold),
