...
const updateProduct = async (req, res) => {
    ...
    try {
        let query;
        if (action === 'sale') {
            query = 'UPDATE products SET name = $1, category = $2, quantity = quantity - $3, reorder_level = $4, expiry = $5 WHERE sku = $6 RETURNING *';
        ...
        const result = await pool.query(query, [name, category, amount, reorder_level, expiry, sku]);
        ...
        if (action === 'sale' && updatedProduct.quantity <= updatedProduct.reorder_level) {
...
const updateQuantity = async (req, res) => {
    ...
    try {
        let query;
        if (action === 'sale') {
            query = 'UPDATE products SET quantity = quantity - $1 WHERE sku = $2 RETURNING *';
        } else if (action === 'return' || action === 'restock') {
            query = 'UPDATE products SET quantity = quantity + $1 WHERE sku = $2 RETURNING *';
        } else {
            return res.status(400).json({ message: 'Invalid action type' });
...
const updateProductbyBarcode = async (req, res) => {
    ...
    try {
        let query;
        if (action === 'sale') {
            query = 'UPDATE products SET quantity = quantity - $1 WHERE barcode = $2 RETURNING *';
        } else if (action === 'return' || action === 'restock') {
            query = 'UPDATE products SET quantity = quantity + $1 WHERE barcode = $2 RETURNING *';
        } else {
            return res.status(400).json({ message: 'Invalid action type' });
