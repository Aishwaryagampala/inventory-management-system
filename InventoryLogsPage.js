...
const InventoryLogsPage = ({ userRole }) => {
    ...
    const fetchProducts = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch('/api/logs/all-logs', { credentials: 'include' }, {
                method: 'GET',
            ...
            if (response.ok) {
                ...
                console.error('Failed to fetch products:', errorData.message || 'Unknown error');
    ...
    useEffect(() => {
        ...
        // Ensure userRole exists before calling toLowerCase()
        if (userRole && userRole.toLowerCase() === 'admin') { // Case-insensitive check for admin
            fetchProducts();
        }
    ...
    const handleProductQuantityUpdateForStaff = () => {
        ...
        console.log("Staff product quantity updated. Consider re-fetching specific product or informing user.");
        ...
        <div className="page-content-container">
            ...
            {!isAdmin && !isStaff && (
                <div style={{ textAlign: 'center', padding: '50px', fontSize: '18px', color: '#666' }}>
                    Access Denied: Please log in with appropriate permissions to view inventory logs.
                </div>
            )}
        </div>
    );
